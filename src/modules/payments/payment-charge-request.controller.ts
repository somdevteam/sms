...
export class PaymentChargeRequestController {
  ...
  @Post()
  async create(
    @Body() createDto: CreatePaymentChargeRequestDto,
  ): Promise<ApiBaseResponse> {
    const chargeRequest = await this.chargeRequestService.create(createDto);
    return new ApiBaseResponse(
      'Payment charge request created successfully',
      200,
      chargeRequest,
    );
  }
  ...
  @Post('filter')
  async findAll(
    @Body() filterDto: PaymentChargeRequestFilterDto,
  ): Promise<ApiBaseResponse> {
    const chargeRequests = await this.chargeRequestService.findAll(filterDto);
    return new ApiBaseResponse(
      'Payment charge requests retrieved successfully',
      200,
      chargeRequests.data,
    );
  }
  ...
  async findOne(@Param('id') id: string): Promise<ApiBaseResponse> {
    ...
    return new ApiBaseResponse(
      'Payment charge request retrieved successfully',
      200,
      chargeRequest,
    );
  }
  ...
  async update(
    ...
    const chargeRequest = await this.chargeRequestService.update(
      +id,
      updateDto,
    );
    return new ApiBaseResponse(
      'Payment charge request updated successfully',
      200,
      chargeRequest,
    );
  }
  ...
  async markAsPaid(@Param('id') id: string): Promise<ApiBaseResponse> {
    ...
    return new ApiBaseResponse(
      'Payment charge request marked as paid successfully',
      200,
      chargeRequest,
    );
  }
  ...
  async generateCharges(
    @Body() generateChargesDto: GenerateChargesDto,
  ): Promise<ApiBaseResponse> {
    const charges = await this.chargeRequestService.generateCharges(
      generateChargesDto,
    );
    return new ApiBaseResponse(
      'Payment charges generated successfully',
      200,
      charges,
    );
  }
  ...
  async getChargeType(@Param('id') id: string): Promise<ApiBaseResponse> {
    ...
    return new ApiBaseResponse(
      'Charge type retrieved successfully',
      200,
      chargeType,
    );
  }
